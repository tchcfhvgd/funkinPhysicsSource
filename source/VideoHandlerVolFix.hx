 ...
 * PARTIAL THANKS TO CC I THINK BECAUSE I GOT THE SMALL CHUNK OF THE VOL FIX CODE FROM THERE MIGHT NOT BE THERE ORIGINALLY THOUGH IDFK I WORK ON THE MOD ANWAY
 * FNEUFHSWEFHUNAWIODWA0DJODDAWPDAWSPDOWKD-0WD39WE08DJWMS,OPX,A
 */
class VideoHandlerVolFix extends VLCBitmap
{
	...
	function fixVolume(e:Event)
	{
		...
		if(!FlxG.sound.muted && FlxG.sound.volume > 0.01) { //Kind of fixes the volume being too low when you decrease it
			volume = Std.int(FlxG.sound.volume * 0.5 + 0.5 * 100);
		}
	}
}
